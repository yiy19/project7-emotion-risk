...
riskraw <- riskraw %>% 
  mutate(
   ...
   gendcond = as.factor(gendcond),
   condition = as.factor(condition),
   risklevel = as.numeric(risklevel),
   pos = as.numeric(pos),
   neg = as.numeric(neg),
   alert = as.numeric(alert),
   riskyrisk = as.numeric(riskyrisk),
   riskavrisk = as.numeric(riskavrisk),
   rel_self = as.factor(rel_self),
   rel_friend1 = as.factor(rel_friend1),
   rel_friend2 = as.factor(rel_friend2),
   attention1 = as.numeric(attention1),
   attention2 = as.numeric(attention2)
  )
# write file
write_sav(riskraw, "data/risk_nonlabelled.sav")
```
